...
const Events = ({navigation}) => {
  ...
  const doctypeFields = useSelector(selectDoctypeFields('Event'));
  const documents = useSelector(selectDocuments({model: 'Event'}));
  ...
  useEffect(() => {
    if (documents.length === 0) {
      const query = {model: 'Event', limit: 1000};
  ...
  const onDeletePress = (_id, title) => {
    setWarning({
      ...
      description: `¿Estás seguro que deseas eliminar el evento llamado: ${title}?`,
    });
  };
  ...
  return (
    <Container>
      <FlatList
        ...
        renderItem={({
          item: {_id, name, title, description, place, date, time},
        }) => (
          <Grid
            title={title}
            description={description}
            bottomLeft={place}
            bottomRight={`${moment(date).format('DD/mm/yyyy')} - ${moment(
              time,
            ).format('hh:mm A')}`}
            source={require('../../assets/images/grids/02.jpg')}
            menu={
              authenticated && [
                {
                  icon: 'edit',
