...
const Audios = ({navigation}) => {
  ...
  const doctypeFields = useSelector(selectDoctypeFields('Audio'));
  const documents = useSelector(selectDocuments({model: 'Audio'}));
  ...
  useEffect(() => {
    if (documents.length === 0) {
      const query = {model: 'Audio', limit: 1000};
  ...
  const onDeletePress = (_id, title) => {
    setWarning({
      ...
      description: `¿Estás seguro que deseas eliminar el audio llamado: ${title}?`,
    });
  };
      ...
      <FlatList
        ...
        renderItem={({item: {_id, name, title, description, link}}) => (
          <Video
            ...
            source={audios[link]}
            poster={require('../../assets/images/logo-circle.png')}
            menu={
              authenticated && [
                {
                  icon: 'edit',
